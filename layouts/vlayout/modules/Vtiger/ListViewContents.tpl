...
<div class="listViewEntriesDiv contents-bottomscroll">
	...
	<table class="table table-bordered listViewEntriesTable">
		...
		<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->getId()}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
			...
			{assign var=LISTVIEW_COUNR value=1}
			{foreach item=LISTVIEW_HEADER from=$LISTVIEW_HEADERS}
			{assign var=LISTVIEW_HEADERNAME value=$LISTVIEW_HEADER->get('name')}
			<td class="listViewEntryValue {$WIDTHTYPE}" data-field-type="{$LISTVIEW_HEADER->getFieldDataType()}" nowrap>
				{if ($MODULE_NAME eq 'ServiceContracts') and ($VIEW eq 'List') and ($LISTVIEW_COUNR == 1)}
					{if strtotime($LISTVIEW_ENTRY->get('createdtime'))+(7*24*60*60) >= time()}
						<img src="layouts/vlayout/skins/images/new.png" style="margin-left: -10px;margin-bottom: -3px;"/>
					{/if}
				{/if}
				{assign var=LISTVIEW_COUNR value=$LISTVIEW_COUNR + 1}
